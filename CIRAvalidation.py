...
ax.grid()
plt.show()
plt.savefig('/home/luka/Dropbox/Thesis/Report/figures/CIRAnozzle.pgf',format="pgf")
...
labs = [line[0].get_label() for line in lins]
lines = [line[0] for line in lins]
ax.legend(lines,labs,loc=0,labelspacing=0,bbox_to_anchor=(1.1, 1.1))
...
ax2.set_ylabel(r"Nu [-]")
plt.show()
plt.savefig('/home/luka/Dropbox/Thesis/Report/figures/CIRAReNu.pgf',format="pgf")
...
ax.grid()
plt.show()
plt.savefig('/home/luka/Dropbox/Thesis/Report/figures/CIRAcoolant.pgf',format="pgf")
...
# Heat fluxes
lins[0] = ax.plot(xVals[1:]*100, np.array(qvals[::-1])/1e6, 'g-',lw=2,  label = r'$q_{tot}$')
lins[1] = ax.plot(xVals[1:]*100, np.array(qradvals[::-1])/1e6, 'r:',lw=2,  label = r'$q_R$')
lins[2] = ax.plot(xVals[1:]*100, (np.array(qvals[::-1])-np.array(qradvals[::-1]))/1e6, 'b-.',lw=2,  label = r'$q_V$')
ax.set_ylim([0,round(max(Twvals)+100,2)])

# Nozzle contour
lins[3] = ax.plot(xVals*100, yVals*100, 'k-', label = r'Contour')
...
# Create labels, show and save
ax.set_xlabel("$x$ coordinate [cm]")
ax.set_ylabel(r"Radius [cm]; $q$ [$\mathrm{MW/m^2}$]")
ax.set_ylim([0,60])
ax.grid()
plt.show()
...
b = interpol(xbCIRA,bCIRA,xVals)
plt.plot(xVals*100,b*1e3,'b-.',lw=2.3,label=r'$w_c$')
#plt.plot(xbCIRA,bCIRA,'b+')
h = interpol(xhCIRA,hCIRA,xVals)
plt.plot(xVals*100,h*1e3,'r--',lw=2.0,label=r'$d_c$')
#plt.plot(xhCIRA,hCIRA,'r+')
s = interpol(xsCIRA,sCIRA,xVals)
plt.plot(xVals*100,s*1e3,'g-',lw=1.5,label=r'$t_i$')
#plt.plot(xsCIRA,sCIRA,'g+')
plt.xlabel(r"$x$ coordinate [cm]")
plt.ylabel(r"$y$ coordinate [mm]")
plt.legend(loc=0,labelspacing=0)
plt.grid()
plt.show()
